...
Last reviewed: 2026-06-02 (Performance hub — merged Portfolio + Logs into one tabbed page; Airtable design system via getdesign)
...
| Trade journal — round-trips / fills / rejected, filters + CSV export | ✅ now the **Round-Trips / Fills / Rejected** tabs of the **Performance hub** (`/performance`); `LogsPage.tsx` renders these in `embedded` mode |
| Market Structure page (LuxAlgo SMC port) | ✅ `/structure`: BOS/CHoCH + structure lines, Strong/Weak, Order Blocks (boxes), EQH/EQL, Fair Value Gaps, Premium/Discount zones, MTF prev D/W/M levels. Internal structure (dashed, length 5), trend-colored candles. Per-layer show/hide toggles, SymbolSearch dropdown, **Setup Summary** panel (bias/zone/nearest OB-FVG-liquidity + plain-language note via `src/lib/smc/summary.ts`), **Strategy Test** panel (`src/lib/smc/strategyTest.ts` — compares **entry × exit combos**: {CHoCH, BOS+CHoCH, CHoCH-in-zone, Retest level/OB/FVG} × {Flip, 2R, MFE}, **ranked by a Quality score 0-100** (`smcQuality`: PF + sample size + win-rate + drawdown; Return% barely weighted because an open position inflates it — sinks PF-0 mirages). Scanner SMC tab ranks by same Quality. Retest entries are now **deployable** — `params.entryMode` (0=break, 1=retest level, 2=retest OB, 3=retest FVG) routes the bot to a self-contained `smcRetestSignals()` that waits for the pull-back after a break and enters on the retest of the broken level / order-block / FVG. That function is a **verbatim copy** in `src/lib/strategies.ts` and `bot/src/strategy/strategies.ts`, locked by a canonical-fixture parity test in BOTH packages (web vitest + `bot/ npm run test:parity` → both assert buys=[46,87] sells=[65]). Every exit is a take-profit % so all run on the **same `runBacktest` engine as the Backtester** with fees → numbers match Backtester AND deployable bot. Deploy card has a 5-option Entry selector (CHoCH / BOS+CHoCH break / Retest level / Retest OB / Retest FVG) + exit selector (Flip / Fixed R:R tpPct / Auto MFE useSuggestedTp), and **Deploy to Signal Bot** (strategy=smc, CHoCH=mode1 / BOS+CHoCH=mode2, entryMode for retest, risk-based sizing, **Auto-TP toggle**). Engine `src/lib/smc/` is pure + unit-tested (Vitest). Also: SMC tab in Scanner ranks coins by best-rule return%. |
| Unit tests (Vitest) | ✅ `npm test` — SMC engine covered; first test suite in the repo |
| Login page redesign (dot-grid, framer-motion) | ✅ |
| Portfolio — account-value trend (HL portfolio), max drawdown, bot leaderboard, Power BI cross-filter | ✅ two-layer model (see gotcha); now the **Overview** tab of the **Performance hub** (`/performance`), `PortfolioPage.tsx` rendered as-is. Old `/logs` + `/portfolio` URLs redirect into the hub. |
