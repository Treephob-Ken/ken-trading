// Custom Strategy evaluator — turns a CustomStrategySpec into a per-bar
// Signal stream that the existing backtest engine can consume directly.
//
// The flow:
//   1. Walk the spec, collect every distinct SeriesRef and SignalEventRef
//      across all condition blocks, and compute each indicator ONCE — so a
//      strategy that uses RSI(14) in three different conditions still only
//      computes RSI(14) one time.
//   2. For each bar, evaluate each block against the cached series, combine
//      via ALL/ANY per group, then map to a long/short/exit signal.
//   3. A simple state machine tracks "flat / long / short" so entries only
//      fire from flat, and exits only fire from the matching side.
...
// Build a stable cache key for a series so identical RSI(14)s in different
// blocks share the same computation.
...
// Resolve a SeriesRef to a numeric array aligned with candles.
function computeSeries(ref: SeriesRef, candles: Candle[]): number[] {
  ...
  switch (ref.id) {
    ...
    case 'ema':
      return ema(closes, Math.max(2, Math.floor(ref.params.length ?? 20)))

    case 'sma':
      return sma(closes, Math.max(2, Math.floor(ref.params.length ?? 20)))
    ...
    case 'stoch_k':
    case 'stoch_d': {
      // kPeriod = lookback for the raw %K; kSmooth smooths %K; dPeriod
      // smooths %D from %K. Defaults match TradingView (14/3/3).
      const s = stochastic(
        highs,
        lows,
        closes,
...
// Resolve a signal event to a boolean array — true on bars where the event
// fired. v1 supports SMC structure events.
function computeEvent(ref: SignalEventRef, candles: Candle[]): boolean[] {
  ...
  switch (ref.kind) {
    ...
    case 'smc_bearish_bos': {
      // SMC's signal mode = 'both' emits CHoCH and BOS but doesn't tell us
      // which is which directly. We need to re-derive that. Easiest: run the
      // structure and tag every bar where a signal fires with whether bias
      // *flipped* (CHoCH) or *continued* (BOS). For now we approximate using
      // the bias-before-and-after the cross — that's what smcStructure does
      // internally. To keep the evaluator simple we just call smcStructure
      // and treat 'buy'/'sell' as 'bos' (which is the superset) when the
      // user picked 'bos'; for CHoCH we replay the logic locally so we can
      ...
      // Run twice — once for the 'both' signal stream (for BOS-or-CHoCH
      // detection), once for 'choch' only (for CHoCH-isolated detection).
      const both = smcStructure(highs, lows, closes, times, swingLength, 'both').signals
      const chochOnly = smcStructure(highs, lows, closes, times, swingLength, 'choch').signals
      ...
      for (let i = 0; i < n; i++) {
        ...
        switch (ref.kind) {
          case 'smc_bullish_choch':
            out[i] = isChoch && chochOnly[i] === 'buy'
            break
          case 'smc_bearish_choch':
            out[i] = isChoch && chochOnly[i] === 'sell'
            break
          case 'smc_bullish_bos':
            // BOS = bothFired AND NOT CHoCH AND direction was 'buy'
            out[i] = bothFired === 'buy' && !isChoch
            break
          case 'smc_bearish_bos':
            out[i] = bothFired === 'sell' && !isChoch
            break
        }
      }
      break
...
function buildCache(spec: CustomStrategySpec, candles: Candle[]): Cache {
  ...
  visit(spec.entryLong)
  visit(spec.exitLong)
  visit(spec.entryShort)
  visit(spec.exitShort)
  return { series, events }
}

// Numeric comparison with NaN-tolerance — any NaN operand → false.
...
// Evaluate one group at bar i. Empty group = false (disabled).
